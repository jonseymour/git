...
test_expect_success 'bisect: --no-checkout --update-ref CURSOR' '
	git update-ref -d CURSOR --no-deref &&
	git bisect reset &&
	git checkout broken &&
...
test_expect_success 'bisect: --update-ref CURSOR --no-checkout # order independent' '
	git update-ref -d CURSOR --no-deref &&
	git bisect reset &&
	git checkout broken &&
	BROKEN=$(git rev-parse broken) &&
	git bisect start broken BROKEN_HASH4 --update-ref CURSOR --no-checkout &&
	check_same CURSOR BROKEN_HASH6 &&
	test "refs/heads/broken" = "$(git rev-parse --symbolic-full-name HEAD)"
'

test_expect_success 'bisect: --update-ref CURSOR # --no-checkout optional' '
	git update-ref -d CURSOR --no-deref &&
	git bisect reset &&
	git checkout broken &&
	BROKEN=$(git rev-parse broken) &&
	git bisect start broken BROKEN_HASH4 --update-ref CURSOR &&
	check_same CURSOR BROKEN_HASH6 &&
	test "refs/heads/broken" = "$(git rev-parse --symbolic-full-name HEAD)"
'
